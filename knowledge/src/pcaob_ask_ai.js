...
function formatDocs(ids, documents, metadatas) {
  ...
  for (const [index, doc] of documents.entries()) {
    text.push(`${metadatas.chapter_title}, page${metadatas.page_number}\n\n${doc}\n`)
  }
  return text.join('\n')
}
...
function findPcaobTopicNumber(str) {
...
async function converse(query) {
  ...
  const topic = findPcaobTopicNumber(query)
  ...
  const pcaob_collection = await client.getCollection(collection_config)
  const result = await pcaob_collection.query({
  ...
  if (topic) {
    const topic_results = await pcaob_collection.get({
      limit: 100,
